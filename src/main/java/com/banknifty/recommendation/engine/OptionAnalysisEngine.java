...
import com.banknifty.service.OpenInterestAnalysisService.OpenInterestTrend;
...
public class OptionAnalysisEngine {
	...
	private void scoreGreeks(OptionCandidate candidate, OptionAnalysis analysis) {
		...
		analysis.setGreekScore(Math.min(score, 5));

		analysis.addScore(Math.min(score, 5));
	...
	private void calculateTradeLevels(OptionCandidate candidate, OptionAnalysis analysis) {
		...
		analysis.setStopLoss(

				premium.multiply(BigDecimal.valueOf(0.85)).setScale(2, RoundingMode.HALF_UP));

		analysis.setTarget1(

				premium.multiply(BigDecimal.valueOf(1.20)).setScale(2, RoundingMode.HALF_UP));

		analysis.setTarget2(

				premium.multiply(BigDecimal.valueOf(1.40)).setScale(2, RoundingMode.HALF_UP));
